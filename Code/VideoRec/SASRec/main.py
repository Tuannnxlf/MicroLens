...
def train(args, model_dir, Log_file, Log_screen, start_time, local_rank):
    ...
    for ep in range(epoch_left):
        ...
        if args.mode == 'test':
            ...
            np.save("/opt/data/private/vllm2rec/MicroLens/statistical_analysis/embedding/finetuned_video_embeddings", all_embs_np)
            print("所有 item 的 finetuned_video_embeddings 形状（含 padding）:", all_embs.shape)  # (item_num + 1, embedding_dim)
            return
